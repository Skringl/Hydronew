...
def result(peleng, Target, Dist_r):
    ...
    TargetR_x = np.sin(np.deg2rad(peleng)) *Dist_r
    TargetR_y = np.cos(np.deg2rad(peleng)) *Dist_r
    ...
    label6=ttk.Label(text=peleng)
    label6.place(x=180,y=420)
    label7=ttk.Label(text=Dist_r)
    label7.place(x=180,y=480)
    label8=ttk.Label(text=Target)
    label8.place(x=180,y=560)
...
random=Button(text = 'Случайно',command = rndm)
